...
if nargin < 3
   ...
   if ~iscell(file)
      if (file == 0)
         disp('No file selected. Canceling extraction.'); 
         data = [];
         meta = [];
         return;
      else
         filename = fullfile(path,file);
         [data,meta] = read_Intan_RHS2000_file(SOS,G,filename);
         return;
      end
   else
      filename = fullfile(path,file);
...
if iscell(filename)
   ...
   for ii = 1:numel(data)
      [data{ii},meta] = read_Intan_RHS2000_file(SOS,G,filename{ii});
   end
   return;
end
...
meta.stim_parameters = stim_parameters;
% if (data_file_main_version_number > 1)
%     data.reference_channel = reference_channel;
% end

if (num_amplifier_channels > 0)
    meta.amplifier_channels = amplifier_channels;
    if (data_present)
       
        data.amplifier_data = amplifier_data(:,(t >= -0.5) & (t <= 0.5));
        spike_data = nan(size(amplifier_data));
        lfp_data = nan(size(amplifier_data));
        for ii = 1:size(amplifier_data,1)
           x = double(amplifier_data(ii,:) - mean(amplifier_data,1));
           spike_data(ii,:) = filtfilt(SOS.spike,G.spike,x);
           lfp_data(ii,:) = filtfilt(SOS.lfp,G.lfp,double(amplifier_data(ii,:)));
        end
        data.spike_data = spike_data(:,(t >= -0.5) & (t <= 0.5));
        data.lfp_data = lfp_data(:,(t >= -0.5) & (t <= 0.5));
%         if (dc_amp_data_saved ~= 0)
%             data.dc_amplifier_data = dc_amplifier_data;
%         end
        data.t = t((t >= -0.5) & (t <= 0.5));
        if nargout > 1
           meta.stim_data = nan(1,2);
           for iCh = 1:size(stim_data,1)
              tmp = find(abs(stim_data(iCh,:))>0,1,'first');
              if ~isempty(tmp)
                 if isnan(meta.stim_data(1,1))
                    meta.stim_data = [iCh,t(tmp)];
                 else
                    meta.stim_data = [meta.stim_data; iCh,t(tmp)];
                 end
              end
           end
        end
...
%         data.compliance_limit_data = compliance_limit_data;
    ...
    meta.spike_triggers = spike_triggers;
end
% if (num_board_adc_channels > 0)
...
%     end
   meta.dig = struct;
   meta.dig.channels = board_dig_in_channels.';
   meta.dig.trig = nan(numel(board_dig_in_channels),1);
   if (data_present)
      for iCh = 1:numel(board_dig_in_channels)
         tmp = t(board_dig_in_data(iCh,:)>0);
         if ~isempty(tmp)
            tmp = tmp(find(tmp>=0,1,'first'));
            meta.dig.trig(iCh) = tmp;
         end
      end
   end
end
